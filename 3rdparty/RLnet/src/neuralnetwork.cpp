...
#include "../include/neural/neuron.h"
#include "../include/misc/functions.h"
...
void NeuralNetwork::addLayer(unordered_map<string, int> parameters)
{
...
double NeuralNetwork::loss(const vector<double>& in, const vector<double>& out)
{
	...
	if (_layers.back()->getParameters().at("activation") == (int)ActivationFunction::SIGMOID)
		for (size_t i = 0; i < out.size(); ++i)
			sum += 0.5 * (out[i] - out_exp[i]) * (out[i] - out_exp[i]);
	if (_layers.back()->getParameters().at("activation") == (int)ActivationFunction::LINEAR)
		for (size_t i = 0; i < out.size(); ++i)
